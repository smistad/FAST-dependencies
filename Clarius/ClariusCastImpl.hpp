 ...
 */
class CLARIUSCASTWRAPPER_EXPORT ClariusCastImpl : public ClariusCast {
    public:
    ...
    /// @retval -1 the connection attempt was not successful
     int Connect(const char* ipAddress, unsigned int port, ClariusReturnFn fn);
    ...
    /// @retval -1 the disconnect was unsuccessful
     int Disconnect(ClariusReturnFn fn);
    ...
    /// @retval -1 the information could not be retrieved
     int ProbeInfo(ClariusProbeInfo* info);
    ...
    /// @retval -1 the callback function could not be set
     int SetRawImageFn(ClariusNewRawImageFn fn);
    ...
    /// @retval 0 the callback function was successfully set
    /// @retval -1 the callback function could not be set
     int SetProcessedImageFn(ClariusNewProcessedImageFn fn);
    ...
    /// @retval 0 the callback function was successfully set
    /// @retval -1 the callback function could not be set
     int SetSpectralImageFn(ClariusNewSpectralImageFn fn);
    ...
    /// @retval 0 the callback function was successfully set
    /// @retval -1 the callback function could not be set
     int SetFreezeFn(ClariusFreezeFn fn);
    ...
    /// @retval 0 the callback function was successfully set
    /// @retval -1 the callback function could not be set
     int SetButtonFn(ClariusButtonFn fn);
    ...
    /// @retval 0 the callback function was successfully set
    /// @retval -1 the callback function could not be set
     int SetProgressFn(ClariusProgressFn fn);
    ...
    /// @retval 0 the callback function was successfully set
    /// @retval -1 the callback function could not be set
     int SetErrorFn(ClariusErrorFn fn);
    ...
    /// @note the probe must be frozen and in a raw data buffering mode in order for the call to succeed
     int RequestRawData(long long int start, long long int end, ClariusReturnFn fn);
    ...
    /// @note the probe must be frozen and a successful call to clariusRequestRawData must have taken place in order for the call to succeed
     int ReadRawData(void** data, ClariusReturnFn fn);
    ...
    /// @retval -1 the call was not successful
     int UserFunction(int cmd, double val, ClariusReturnFn fn);
};


extern "C" CLARIUSCASTWRAPPER_EXPORT 
ClariusCast* load() {
    return new ClariusCastImpl();
}
